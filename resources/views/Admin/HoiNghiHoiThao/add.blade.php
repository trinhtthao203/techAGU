...
@section('title', __('Thêm mới Hội nghị Hội thảo'))
@section('css')
...
<div class="row">
    <div class="col-12">
        <div class="card-box">
            <h3 class="m-t-0"><a href="{{ env('APP_URl') }}{{ app()->getLocale() }}/admin/hoi-nghi-hoi-thao" class="btn btn-primary btn-sm"><i class="mdi mdi-reply-all"></i> {{ __('Trở về') }}</a> {{ __('Thêm mới Hội nghị Hội thảo') }}</h3>
            <form action="{{ env('APP_URL') }}{{ app()->getLocale() }}/admin/hoi-nghi-hoi-thao/create" method="post" id="dinhkemform" enctype="multipart/form-data">
                {{ csrf_field() }}
                ...
                <div class="form-body">
                    ...
                    @php
                        if(old('ten') != null){
                            $ten = old('ten'); $noi_dung = old('noi_dung');$slug = old('slug');$mo_ta = old('mo_ta');
                        } else if(isset($ds['ten']) && $ds['ten']){
                            $ten = $ds['ten']; $noi_dung = $ds['noi_dung'];$slug = $ds['slug'];$mo_ta = $ds['mo_ta'];
                        } else {
                            $ten = '';$mo_ta = '';$noi_dung = '';$slug='';
                        }
                    @endphp
                ...
                <div class="form-actions">
                    <a href="{{ env('APP_URL') }}{{ app()->getLocale() }}/admin/hoi-nghi-hoi-thao" class="btn btn-light"><i class="fa fa-reply-all"></i> {{ __('Trở về') }}</a>
