            ...
            <table class="table table-border table-striped">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $key+1 }}</td>
                  <td>
                    @if(strtolower($dk['type']) == 'pdf') <a href="{{ env('APP_URL').app()->getLocale() }}/hoi-nghi-hoi-thao/xem-truc-tuyen/{{ $ds['_id'] }}/{{ $key }}" data-toggle="modal" data-target="#xemdinhkem" class="view_online"> @endif
                      {{ $dk['title'] }}
                    @if(strtolower($dk['type']) == 'pdf') </a> @endif
                  </td>
                  <td class="text-center">
                    <a href="{{ env('APP_URL').app()->getLocale() }}/hoi-nghi-hoi-thao/tai-ve/{{ $ds['_id'] }}/{{ $key }}">
          ...
          <div class="col-md-12">
            ...
            <ul class="list-quy-che">
              @foreach($danhsach as $r)
                <li class="rtejustify"><a href="{{ env('APP_URL').app()->getLocale() }}/hoi-nghi-hoi-thao/{{ $r['slug'] }}">{{ $r['ten'] }}</a></li>
              @endforeach
            </ul>
          </div>
        </div>
