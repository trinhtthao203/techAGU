  ...
  <div class="container">
    <div class="row">
      <div class="col-12 col-md-12">
        <h3 style="padding-bottom:20px;"><i class="fa fa-handshake-o"></i> {{ __('Đối tác') }}</h3>
      </div>
    </div>
    @if($danhsach)
              ...
              <div class="inner">
                ...
                @endif
                  <div class="cnt-block">
                    ...
                    <h2 style="height:80px;overflow:hidden;"><a href="{{ env('APP_URL').app()->getLocale() }}/doi-tac/{{ $ds['slug'] }}" title="{{ $ds['ten'] }}">{{ Str::limit($ds['ten'],100) }}</a></h2>
                    <p  style="height:100px;overflow:hidden;">{{ $ds['mo_ta'] }}</p>
                    <br />
                    <a href="{{ env('APP_URL') }}{{ app()->getLocale() }}/doi-tac/{{ $ds['slug'] }}" class="read-more"><span class="icon-play-icon"></span>{{ __('Xem thêm') }}</a>
                </div>
            </div>
          </li>
          @endforeach
        </ul>
        {{ $danhsach->withPath(env('APP_URL').app()->getLocale() . '/doi-tac') }}
    @endif
  </div>
</section>
@endsection
