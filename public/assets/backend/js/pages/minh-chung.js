function dm_minhchung(app_url){
	$(".dm_minhchung").change(function(){
		var _this = $(this);
		var value = _this.val();
		var path = app_url + value + "/" + value;
		var id = _this.attr("data-id");
		var html = '<option value=""></option>';
		if(id == 'dm_minhchung_2'){
			$("#dm_minhchung_2").html(html);$("#dm_minhchung_3").html(html);$("#dm_minhchung_4").html(html);
		}
		if(id == 'dm_minhchung_3'){
			$("#dm_minhchung_3").html(html);$("#dm_minhchung_4").html(html);
		}
		$("#minhchungForm").submit();
	});
}

function dm_minhchung_autoload(app_url, id_minhchung){
	$.get(app_url, function(data){
		$("#"+id_minhchung).html(data);
	});
}
...
function dinh_kem_button(path, path_donvi){
	$("#dinh-kem-button").change(function(){
		var mc_1 = $("#dm_minhchung_1").val();
		if(mc_1){
			//$("#minhchungForm").submit();
            upload_files(path, path_donvi);
		} else {
			alert("Vui lòng chọn Tiêu chuẩn, Tiêu chí");
		}
	});
}
